...
check <- c("quarto", "ranger", "sf", "shiny", "bslib", "vip",
           "tidymodels", "modeltime", "timetk", "factoextra", "cluster")

data.frame(
  package = check,
  in_lock = check %in% pkgs,
  version = sapply(check, function(p) if (p %in% pkgs) lock$Packages[[p]]$Version else NA)
)

length(pkgs)

renv::record(c("quarto"))  # only the missing ones
renv::snapshot() 
"quarto" %in% names(renv::lockfile_read()$Packages) 

file.exists("renv/.gitignore")
readLines("renv/.gitignore")
